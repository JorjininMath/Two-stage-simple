"""
tune_plugin_sigma.py

Scale-only tuning for the plug-in adaptive-h estimator.
...
Recommended use:
    python exp_adaptive_h/tune_plugin_sigma.py --simulators exp2_gauss_low
    python exp_adaptive_h/tune_plugin_sigma.py --simulators exp2_gauss_low,wsc_gauss,nongauss_A1L
"""
...
from exp_adaptive_h.adaptive_h_utils import ORACLE_SCALE
...
def _evaluate_one(
    ...
    if cfg["d"] != 1:
        raise ValueError("tune_plugin_sigma currently supports 1D simulators only")
    ...
    for bw_factor in bw_factors:
        est = PluginSigma.fit(X_all, Y_all, n_0=n_0, r_0=r_0, bw_factor=bw_factor)
...
def _plot_summary(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    for ax in axes:
        ax.set_xlabel("PluginSigma bw_factor")
        ax.set_xscale("log")
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)

    fig.suptitle("Plug-in scale tuning: kernel smoothing bandwidth", y=1.02)
    fig.tight_layout()
    fig.savefig(out_dir / "plugin_sigma_bw_tuning.png", dpi=150, bbox_inches="tight")
    plt.close(fig)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Tune PluginSigma kernel bandwidth against oracle scale functions."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="exp_adaptive_h/output_plugin_sigma_tuning",
    )
    ...
    per_path = out_dir / "plugin_sigma_tuning_per_macro.csv"
    sum_path = out_dir / "plugin_sigma_tuning_summary.csv"
