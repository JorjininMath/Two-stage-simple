# exp_coverage_mechanism — Specification

Status: spec for implementation, 2026-07-14. Implements the falsifiable claims
from `notes/planning/au_eu_anatomy_note.md` (local note) and feeds the theory
...
General conventions (match `exp_adaptive_h/spec.md` where applicable):
...
- Plug-in scale estimator = site sample SD + Gaussian Nadaraya-Watson
  smoothing (`s_hat_NW`), exactly as in the final `exp_adaptive_h` spec.
  No IQR.
- Do not modify `CKME/`, `CP/`, `Two_stage/` core APIs. New simulators are
  registered in `Two_stage/sim_functions/` + `__init__.py`. English code and
  comments.

Implementation-alignment requirements (checked 2026-07-15, must hold in all
new runners):

- **Query-specific bandwidth (confirmed in `exp_adaptive_h/adaptive_h_utils.py`):**
...
- **Two-layer evaluation protocol (`notes/planning/cdf_legality_policy.md` §6):**
...
`CKME/indicators.py`, g0 = standard normal, kappa_2 = 1) and Gaussian noise.
...
simulator docstrings). Register both in `Two_stage/sim_functions/`.
...
| `ckme_plugin` | DCP score, h(x) = c * s_hat_NW(x) | our method (c from exp_adaptive_h default) |
...
`exp_adaptive_h` — both methods see identical information.
...
in `exp_coverage_mechanism/pilot_score_design/`.
...
- E2 is the heavy one: 50 macroreps x 3 DGPs x shared fit + 5 scoring arms.
  ...
  weights where the implementation allows, as in exp_adaptive_h). Parallel
  by macrorep (`--n_workers`); SLURM array optional, follow
  `exp_gibbs_compare/run_all_gibbs_arc.sh` pattern if needed.
...
1. `Two_stage/sim_functions/sim_shape_t_dfx.py`, `sim_shape_gamma_kx.py`,
...
2. `exp_coverage_mechanism/run_e1_distortion.py` (E1a/E1b/E1c via
   `--part {a,b,c,all}`), `run_e2_shape.py`, `run_e3_validity.py`,
   `summarize.py`, `plot_e1.py`, `plot_e2.py`, `plot_e3.py`.
3. `exp_coverage_mechanism/pretrain_params.py` producing
...
5. README.md in the folder summarizing commands (follow
   `exp_framing_validation/README.md` style).
