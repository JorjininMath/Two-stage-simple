"""
exp2_test: Compare CKME, DCP-DR, hetGP on exp2_test (exp2 true function + Student-t noise).
...
import numpy as np
import pandas as pd
from exp2_test.config_utils import load_config_from_file, get_config, get_x_cand
...
def run_one_macrorep(
    ...
    """Run one macrorep: Stage1 + Stage2 (one case).

    If return_df is False (default), return per-method mean coverage/width/score (dict).
    If return_df is True, return (summary_dict, per_point_df) where per_point_df has
    coverage/width etc for CKME, DCP-DR, hetGP at each test point.
    """
...
def main():
    parser = argparse.ArgumentParser(description="exp2_test: compare CKME, DCP-DR, hetGP (cov, width, score)")
    ...
    summary_path = out_dir / "exp2_compare_summary.csv"
