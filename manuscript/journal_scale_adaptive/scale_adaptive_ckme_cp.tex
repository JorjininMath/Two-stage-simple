...
% The numerical section uses only the QA-checked final benchmark assets.
\newif\ifincludeexperiments
\includeexperimentstrue

\title{Scale-Adaptive Distributional Conformal Prediction with\\
Conditional Kernel Mean Embeddings for Stochastic Simulation Outputs}
...
\begin{abstract}
Predictive inference for stochastic simulation must provide reliable coverage for future outputs while remaining informative under heteroscedastic and non-Gaussian response distributions. Conditional kernel mean embedding distributional conformal prediction (CKME-DCP) estimates the conditional cumulative distribution function (CDF) through smoothed indicator functions. Its output-side bandwidth is therefore part of the statistical procedure: a fixed bandwidth \(h\) induces the input-dependent standardized smoothing ratio \(h/s(x)\), where \(s(x)\) is the local response scale. We introduce the scale-adaptive rule \(h(x)=c\widehat s(x)\), with \(\widehat s(x)\) estimated from replicated training simulations. Under a heteroscedastic location-scale model, oracle smoothing \(h(x)=c s(x)\) removes this input-dependent distortion and makes the smoothed-oracle DCP score homogeneous across inputs. We also separate CKME estimation error from smooth-indicator bias and show how a consistent response-scale pattern transfers the oracle mechanism to the plug-in procedure. The coverage layer remains standard split conformal prediction: after specifying a target input law \(q_X\), iid calibration and future pairs from the induced target joint law give finite-sample marginal validity.
\ifincludeexperiments
In a paired benchmark with 50 macroreplications, all three bandwidth rules retain approximately 90\% raw-score marginal coverage. Oracle scale normalization substantially reduces the score-heterogeneity diagnostic on the Gaussian and heavy-tailed raised-floor models. The sample-SD plus Nadaraya--Watson plug-in's raw relative scale error declines as the training budget grows, and its score diagnostic moves toward the oracle in those two models, but its interval-score and groupwise gains are data-generating-process dependent.
\fi
\end{abstract}
...
This paper focuses on the second issue while treating the target distribution as an explicit validity protocol. We use CKME as the conditional distribution estimator because it estimates a full conditional law as an RKHS-valued object and evaluates conditional CDFs through smooth test functions. This representation exposes a natural scale-adaptive smoothing rule,
\[
  h(x)=c\widehat s(x),
\]
where \(\widehat s(x)\) is estimated from training-stage simulation replications. The result is a conformal procedure that keeps the finite-sample split-CP guarantee at the target-distribution level while targeting conditional efficiency through better-scaled local CDF smoothing.

The contributions are as follows.
\begin{enumerate}[leftmargin=1.7em]
  \item We introduce scale-adaptive CKME-DCP. It replaces a single smooth-indicator bandwidth with \(h(x)=c\widehat s(x)\), where \(\widehat s(x)\) is learned from replicated training outputs and smoothed across input space.
  \item We show that, under a heteroscedastic location-scale model, oracle adaptive smoothing \(h(x)=c s(x)\) removes the input-dependent standardized ratio \(h/s(x)\) induced by fixed smoothing and restores homogeneity of the smoothed-oracle DCP score. We then connect the result to the implementable sample-SD plus Nadaraya--Watson estimator.
  \item We retain the CKME conditional-CDF error decomposition into embedding estimation error and smooth-indicator bias, and show how uniform CDF error transfers to DCP score and threshold error. Split conformal validity remains estimator-agnostic when calibration and target pairs are exchangeable under the stated target law \(q_XP_{Y\mid X}\).
  \ifincludeexperiments
  \item We report a 50-macroreplication paired benchmark under iid target-law calibration. Oracle scaling substantially reduces the score-heterogeneity diagnostic on two raised-floor location-scale models, and the plug-in increasingly tracks the response scale, while the same benchmark identifies settings in which it does not improve interval score or groupwise coverage over fixed bandwidth.
  \fi
\end{enumerate}

This manuscript builds on our WSC 2026 conference manuscript, \emph{Distributional Conformal Prediction with Conditional Kernel Mean Embeddings for Simulation Output Analysis}. The conference version develops fixed-bandwidth CKME-DCP and its conditional-CDF error bound. The present manuscript adds the scale-adaptive bandwidth, its oracle and plug-in theory, iid target-law calibration, the two-layer numerical evaluation protocol, and a new paired benchmark. The diagnostic adaptive calibration-site design from the conference workflow is not used here.
...
The choice of score determines the geometry and efficiency of the resulting prediction set. Residual scores are simple, but they often behave like a global-scale correction. Locally normalized conformal methods divide a residual score by an estimated difficulty or noise scale so that interval width can vary with the input \cite{papadopoulos2011}. Conformalized quantile regression uses estimated conditional quantiles to construct locally adaptive intervals \cite{romano2019}. Distributional conformal prediction instead estimates a conditional CDF and calibrates a score on the probability scale \cite{chernozhukov2021}. Our scale correction differs from residual normalization: it changes the smooth-indicator bandwidth inside the CKME conditional-CDF estimator while retaining the DCP score \(A\{\widehat F(y\mid x)\}\). This distinction matters because the proposed mechanism targets standardized CDF smoothing rather than a residual denominator.

The present paper follows the distributional route because simulation analysts often need more than two quantiles: they may want the full conditional output distribution, interval inversion at multiple confidence levels, tail probabilities, or service-level probabilities. CKME is useful in this setting because it represents the conditional law as a single object rather than as a separate regression for every threshold or quantile.
...
\section{Target-Law Calibration Protocol}
\label{sec:target}
...
\begin{equation}
  ...
  \label{eq:dcp-score}
\end{equation}
Here and below, \(\widehat F(y\mid x)\) in the conformal score denotes the direct point evaluation clipped to \([0,1]\), without monotone projection over a response grid.
...
The simulation training design contains \(n_0\) distinct sites with \(r_0\) independent replications per site:
\[
  \Dcal_{\mathrm{tr}}
  =
  \{(x_i,Y_{ij}):i=1,\ldots,n_0,\ j=1,\ldots,r_0\}.
\]
Define the sitewise empirical output embedding
\[
  \overline\phi_i
  =
  \frac{1}{r_0}\sum_{j=1}^{r_0}\phi_Y(Y_{ij}).
\]
The estimator implemented in this paper is the distinct-site form of vector-valued kernel ridge regression,
\[
  \widehat\mu_{Y\mid x}
  =
  \sum_{i=1}^{n_0}a_i(x)\overline\phi_i,
  \qquad
  a(x)=(K_0+n_0\lambda I)^{-1}k_0(x),
\]
where \(K_0\) is the \(n_0\times n_0\) Gram matrix with entries
\([K_0]_{i\ell}=k_X(x_i,x_\ell)\), and
\[
  k_0(x)=\big(k_X(x_1,x),\ldots,k_X(x_{n_0},x)\big)^\top .
\]
Here \(\lambda>0\) is the regularization parameter and \(I\) is the identity matrix. This expression is algebraically equivalent to fitting the replicated \(n_0r_0\)-row data matrix with regularization \(n_0r_0\lambda\): the repeated-input block structure makes coefficients equal within a site, and the \(r_0\) factor cancels after sitewise averaging. The distinct-site system is used because it reduces the Gram matrix from \(n_0r_0\) rows to \(n_0\) rows without changing the fitted CDF. Substituting \(\widehat\mu_{Y\mid x}\) into the smoothed CDF representation gives
\begin{equation}
  \widehat F_h(t\mid x)=
  ...
  \sum_{i=1}^{n_0}a_i(x)
  \left\{
  \frac{1}{r_0}\sum_{j=1}^{r_0}
  \Psi\!\left(\frac{t-Y_{ij}}{h}\right)
  \right\}.
...
\begin{equation}
  ...
  \left\{
  \frac{1}{r_0}\sum_{j=1}^{r_0}
  \Psi\!\left(\frac{t-Y_{ij}}{h(x)}\right)
  \right\}.
...
The coefficient vector \(a(x)\) is not normalized and may contain negative
entries. The output RKHS motivates the conditional-embedding representation,
but the numerical implementation does not materialize an output Gram matrix:
the reproducing identity reduces evaluation to the scalar values
\(g_{t,h(x)}(Y_{ij})\). Consequently, the membership and bounded-norm
conditions imposed below on the smoothed-indicator class are analytical
assumptions; they are not enforced by the software.
...
\]
then smooth \((x_i,\widehat s_i)\) over \(\Xcal\) using Nadaraya--Watson regression with a Gaussian kernel \cite{nadaraya1964,watson1964,hardle1990}:
\[
  \widehat s(x)
  =
  \max\left\{
  s_{\min},
  \frac{\sum_{i=1}^{n_0}w_i(x)\widehat s_i}
       {\sum_{i=1}^{n_0}w_i(x)}
  \right\},
  \qquad
  w_i(x)
  ...
  \exp\left\{
  -\frac12\sum_{\ell=1}^d
  \left(\frac{x_\ell-x_{i\ell}}{b_\ell}\right)^2
  \right\}.
\]
The implementation uses \(s_{\min}=10^{-3}\). The final benchmark freezes
\[
  b_\ell
  =
  1.06
  \min\left\{
  \operatorname{SD}(x_{1\ell},\ldots,x_{n_0\ell}),
  \frac{\operatorname{IQR}(x_{1\ell},\ldots,x_{n_0\ell})}{1.34}
  \right\}
  n_0^{-1/5},
\]
the Silverman rule-of-thumb input bandwidth with factor one
\cite{silverman1986}. The IQR in this formula only stabilizes the
\emph{input-space} smoothing bandwidth; the response-scale observations being
smoothed are always the sitewise sample standard deviations. The retired IQR
response-scale plug-in is not used. The bandwidth \(h(x)=c\widehat s(x)\) is
then used both for calibration scores and for prediction-set inversion.

Under the location-scale model, write the standardized replications at a site as \(\varepsilon_{i1},\ldots,\varepsilon_{ir_0}\), and define
\[
  V_{i,r_0}
  =
  \left\{\frac{1}{r_0-1}\sum_{j=1}^{r_0}
  \bigl(\varepsilon_{ij}-\overline\varepsilon_i\bigr)^2\right\}^{1/2},
  \qquad
  \kappa_{r_0}=\E[V_{i,r_0}].
\]
Then \(\widehat s_i=s(x_i)V_{i,r_0}\). With fixed \(r_0\), Nadaraya-Watson smoothing therefore targets \(\kappa_{r_0}s(x)\), not necessarily \(s(x)\) itself. The factor \(\kappa_{r_0}\) is deterministic and independent of \(x\) when the standardized noise law is input-invariant, so it does not alter the learned scale pattern and can be absorbed into the global multiplier \(c\). The purpose of \(\widehat s(x)\) is to learn this local scale pattern from designed replications rather than to estimate every feature of the conditional output distribution.
...
\begin{enumerate}[leftmargin=1.7em]
  ...
  \item Independently draw calibration inputs from \(q_X\), collect one output at each input, and compute the direct point-evaluated scores
  \[
    \widehat S_i
    =
    \abs{
    \operatorname{clip}_{[0,1]}
    \!\left\{\widehat F_{h(\cdot)}
    (Y_i^{\mathrm{cal}}\mid X_i^{\mathrm{cal}})\right\}
    -1/2}.
  \]
  These raw point scores do not use a response-grid projection.
  \item Let \(\widehat q_{1-\alpha}\) be the split-conformal quantile of \(\widehat S_1,\ldots,\widehat S_{n_{\mathrm{cal}}}\).
  \item Evaluate guarantee-bearing coverage by raw score membership:
  \[
    \ind\!\left[
    \abs{
    \operatorname{clip}_{[0,1]}
    \!\left\{\widehat F_{h(\cdot)}(y\mid x)\right\}
    -1/2}
    \le \widehat q_{1-\alpha}
    \right].
  \]
  \item For interval width and interval score, monotonize the CDF values on the response grid and report their generalized-inverse interval, as described below.
\end{enumerate}

\subsection{Implementation details}
\label{sec:implementation}

The implementation retains the training-calibration split of the WSC version but changes three parts of the workflow: calibration inputs are iid draws from the target law with one output per input, the output-side bandwidth is evaluated as a function of the prediction input, and guarantee-bearing score membership is separated from projected-interval reporting. The CKME hyperparameters \((\ell_x,\lambda)\) are selected using the training data. The fixed-bandwidth baseline also tunes a scalar \(h\). The adaptive version replaces this scalar with \(h(x)=c\widehat s(x)\), using the same CKME input weights \(a(x)\). Thus the computationally expensive distinct-site matrix solve for \((K_0+n_0\lambda I)^{-1}\) is unchanged.

For the final benchmark, the input kernel and smooth step are
\[
  k_X(x,x')
  =
  \exp\left\{
  ...
  \Psi(u)=\{1+\exp(-u)\}^{-1}.
\]
The logistic argument is clipped to \([-50,50]\) only for numerical
stability. For each DGP, the triplet
\((\ell_x,\lambda,h_{\mathrm{fixed}})\) is selected once by site-grouped
CRPS cross-validation on a separate pretraining sample and then frozen across
budgets and macroreplications. The oracle and plug-in arms retain the same
\((\ell_x,\lambda)\) and replace only the output-side bandwidth.

The implementation uses two distinct evaluation layers. The guarantee layer calibrates and evaluates the same raw point score, without a response-grid projection. This score membership is the primary coverage metric. The reporting layer first forms the monotone projected CDF on a response grid \(\Tcal=\{t_1<\cdots<t_M\}\):
\[
  \widetilde F(t_j\mid x)
  =
  \operatorname{clip}_{[0,1]}
  \left\{\max_{1\le k\le j}
  \widehat F_{h(\cdot)}(t_k\mid x)\right\}.
\]
It then reports the generalized-inverse endpoints
\[
  L(x)=\inf\{t_j:\widetilde F(t_j\mid x)\ge 1/2-\widehat q_{1-\alpha}\},
  \qquad
  U(x)=\inf\{t_j:\widetilde F(t_j\mid x)\ge 1/2+\widehat q_{1-\alpha}\}.
\]
Width and interval score are computed only from \([L(x),U(x)]\). Because the raw score set can be disconnected when the unprojected CKME CDF is nonmonotone, containment in this reporting interval need not coincide with raw score membership. Any discrepancy between the two layers is therefore reported rather than treated as part of the conformal guarantee.

The implementation retains the flattened training arrays for data handling but uses the distinct-site system and sitewise smooth-indicator averages in \eqref{eq:ckme-cdf-fixed}--\eqref{eq:ckme-cdf-adaptive}. The same replication structure supplies the sitewise sample standard deviations used to estimate \(\widehat s\). Conformal calibration scores are computed at the individual-output level on a separate iid sample from \(q_X\), independent of the training sample used to fit CKME and estimate \(\widehat s\).
...
\]
All theoretical comparisons below are made over a response range \(\Tcal\). For a strict compact-range statement, assume \(\Pp(Y\in\Tcal)=1\) under the target law. For an unbounded output, this assumption is not equivalent to choosing a numerically wide grid. One must instead take \(\Tcal=\R\) when the uniform conditions hold globally, or use expanding ranges \(\Tcal_n\uparrow\R\) and track
\[
  \delta_{\Tcal_n}=\Pp(Y\notin\Tcal_n).
\]
A marginal probability bound obtained on \(\{Y\in\Tcal_n\}\) then incurs an additional term no larger than \(\delta_{\Tcal_n}\); a groupwise result incurs the corresponding maximum group-conditional tail probability. The numerical guarantee-bearing score is evaluated directly at each observed \(Y\), including values outside the finite response grid; only projected-interval reporting uses that grid.
...
Suppose Assumptions~\ref{ass:uniform}--\ref{ass:score} hold, and let
\(\delta_{\Tcal}=\Pp(Y\notin\Tcal)\) under the target law. Then
\[
  \sup_{x\in\Xcal,\ y\in\Tcal}
...
\[
  ...
  =
  \Op\!\left(
  \epsilon_n+n_{\mathrm{cal}}^{-1/2}+\delta_{\Tcal}
  \right).
\]
For compactly supported outputs, \(\delta_{\Tcal}=0\). For unbounded outputs,
the same statement applies to expanding \(\Tcal_n\) when
\(\delta_{\Tcal_n}\to0\).
\end{proposition}
...
\begin{assumption}[Uniform relative scale-pattern consistency]
\label{ass:scale}
For a fixed \(r_0\ge 2\), suppose \(0<\kappa_{r_0}<\infty\), where \(\kappa_{r_0}\) is the expected standardized sample standard deviation defined in Section~\ref{sec:local-scale}. The plug-in scale estimator satisfies
\[
  \Delta_n
  =
  \sup_{x\in\Xcal}
  \abs{\frac{\widehat s(x)}{\kappa_{r_0}s(x)}-1}
  =\op(1).
\]
\end{assumption}

Assumption~\ref{ass:scale} is a high-level condition on the full scale estimator, not a consistency theorem proved here. Standard Nadaraya--Watson results motivate it under regular design density, smoothness, moment, and bandwidth conditions \cite{hardle1990}; the fixed-\(r_0\) sitewise sample standard deviations act as noisy regression responses with conditional mean \(\kappa_{r_0}s(x)\). Boundary correction and higher-dimensional rates require additional conditions.
...
\[
  ...
  \right]
\]
and let \(\bar c=c\kappa_{r_0}\). Let \(F_{\bar c}^\circ\) be the oracle adaptive smoothed CDF in \eqref{eq:adaptive-representation} with \(c\) replaced by \(\bar c\). Then
\[
  \sup_{x\in\Xcal,\ t\in\Tcal}
  \abs{F_{\widehat h}^{\circ}(t\mid x)-F_{\bar c}^\circ(t\mid x)}
  =
  \Op(\Delta_n)
  =
  \op(1).
\]
Thus fixed-\(r_0\) sample-SD smoothing is asymptotically equivalent, under
Assumption~\ref{ass:scale}, to oracle scale-adaptive smoothing with a
different global multiplier. Since \(\bar c\) is constant in \(x\),
\(\kappa_{r_0}\) can in principle be absorbed into the scalar tuning
parameter and does not affect oracle score homogeneity. In the final
benchmark, however, both the oracle and plug-in arms use the unadjusted
default \(c=1\), so their finite-sample global-multiplier difference remains.
\end{proposition}
...
\begin{assumption}[Smooth-step approximation to the exact CDF]
\label{ass:smoothing-bias}
The conditional density \(f(\cdot\mid x)\) exists and satisfies
\[
  \sup_{x\in\Xcal,\ y\in\R}f(y\mid x)\le B<\infty.
\]
Moreover,
\[
  C_\Psi
  =
  \int_{-\infty}^{\infty}
  \abs{\Psi(u)-\ind\{u\ge0\}}\,du
  <\infty.
\]
\end{assumption}

\begin{proposition}[CKME estimation error and smooth-indicator bias]
\label{prop:ckme-exact-cdf}
Under Assumptions~\ref{ass:ckme}--\ref{ass:smoothing-bias}, define
\[
  C_{h(\cdot)}
  =
  \sup_{x\in\Xcal,\ t\in\Tcal}
...
\begin{aligned}
  &\sup_{x\in\Xcal,\ t\in\Tcal}
  \abs{\widehat F_{h(\cdot)}(t\mid x)-F(t\mid x)}\\
  &\quad\le
  C_{h(\cdot)}
  \sup_{x\in\Xcal}
  \norm{\widehat\mu_{Y\mid x}-\mu_{Y\mid x}}_{\Hcal_Y}
  +
  B C_\Psi\sup_{x\in\Xcal}h(x).
\end{aligned}
\]
For fixed \(h\), this reduces to the WSC error decomposition
\[
  \sup_{x,t}\abs{\widehat F_h(t\mid x)-F(t\mid x)}
  \le
  C_h\sup_x
  \norm{\widehat\mu_{Y\mid x}-\mu_{Y\mid x}}_{\Hcal_Y}
  +
...
Proposition~\ref{prop:ckme-exact-cdf} separates estimation error from smoothing bias relative to the exact CDF. The adaptive-homogeneity analysis instead targets \(F_{h(\cdot)}^\circ\), because \(h(x)=c s(x)\) with fixed \(c\) need not vanish. Combining Proposition~\ref{prop:ckme-cdf} with Proposition~\ref{prop:error-transfer} gives score and threshold stability relative to that smoothed oracle. Combining those results with Theorem~\ref{thm:homogeneity} yields the following group-conditional implication.

\begin{theorem}[Asymptotic finite-partition group coverage]
\label{thm:group}
Suppose the location-scale model holds, oracle adaptive smoothing \(h(x)=c s(x)\) is used, \(n_0,n_{\mathrm{cal}}\to\infty\), and Assumptions~\ref{ass:uniform}, \ref{ass:score}, \ref{ass:ckme}, \ref{ass:g}, and \ref{ass:group} hold with \(F^\circ=F_c^\circ\). Assume either that \(Y\in\Tcal\) almost surely, that the uniform conditions hold on \(\R\), or that expanding ranges satisfy
\[
  \max_{1\le k\le K}
  \Pp(Y\notin\Tcal_n\mid X\in\Gcal_k)\to0.
\]
Then
\[
  \max_{1\le k\le K}
...
Suppose the location-scale model holds, the plug-in bandwidth \(h(x)=c\widehat s(x)\) is used, \(n_0,n_{\mathrm{cal}}\to\infty\), and Assumptions~\ref{ass:score}, \ref{ass:scale}, \ref{ass:std-moment}, \ref{ass:ckme}, \ref{ass:g}, and \ref{ass:group} hold with the oracle multiplier \(\bar c=c\kappa_{r_0}\). Also suppose the response-range condition of Theorem~\ref{thm:group} holds. If the CKME estimator is uniformly consistent for the plug-in smoothed oracle \(F_{\widehat h}^{\circ}\), then the conclusion of Theorem~\ref{thm:group} continues to hold:
\[
  \max_{1\le k\le K}
  \abs{
  \Pp\{Y\in\widehat C(X)\mid X\in\Gcal_k\}
...
\subsection{Locked benchmark and paired design}

The final benchmark follows the matched target-law protocol used in the validity result. Stage~1 uses an equally spaced input grid, \(r_0=10\) replications per site, and budgets \(B=n_0r_0\in\{100,250,500,1000\}\). For every macroreplication, the calibration and test inputs are fresh iid draws from the same uniform target law \(q_X\). Each of the \(n_{\mathrm{cal}}=1000\) calibration inputs and \(n_{\mathrm{test}}=1000\) test inputs receives one independent simulator output. The diagnostic site-selection score \(S^0\) is not used.

We compare three bandwidth rules on exactly the same Stage-1, calibration, and test data within a macroreplication. The fixed arm uses the scalar \(h\) selected once during separate site-grouped pretraining. The oracle arm uses \(h(x)=s(x)\), and the implementable arm uses \(h(x)=\widehat s(x)\), where \(\widehat s\) is the Gaussian Nadaraya--Watson smoother of the Stage-1 sample standard deviations. The multiplier is fixed at \(c=1\), and the input-space smoothing bandwidth uses a frozen Silverman rule with factor one. Unlike the fixed arm, the plug-in multiplier and its input-space bandwidth factor are not tuned. This is therefore a frozen-default mechanism benchmark: it compares input-dependent bandwidth forms under a locked protocol, but it does not isolate local scale adaptation from global-multiplier tuning or constitute a comprehensive external-baseline comparison. All reported means and Monte Carlo standard errors use 50 independent macroreplications.

The three one-dimensional data-generating processes (DGPs) are
\begin{align*}
  \text{M/M/1:}\quad&
  Y\mid \rho\sim\operatorname{Exponential}(\text{rate}=1-\rho),
  &&\rho\in[0.1,0.9],\quad s(\rho)=\frac{1}{1-\rho},\\
  \text{Gaussian:}\quad&
  Y=e^{x/10}\sin x+s(x)\varepsilon,
  &&\varepsilon\sim N(0,1),\\
  \text{Student-}t_3:\quad&
  Y=e^{x/10}\sin x+s(x)\varepsilon,
  &&\varepsilon\sim t_3/\sqrt{3},
\end{align*}
where \(x\in[0,2\pi]\) and \(s(x)=0.10+0.20(x-\pi)^2\) for the last two models. The variance normalization makes \(s(x)\) the conditional standard deviation in both raised-floor models.
\FloatBarrier

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{final_scale_functions.pdf}
\caption{Conditional response scales in the three final benchmark DGPs. The oracle arm uses these functions; the plug-in arm estimates their pattern using only replicated Stage-1 outputs.}
\label{fig:final-scales}
\end{figure}

Coverage is evaluated by membership under the raw point score, which is the guarantee-bearing layer. We separately report coverage of the projected-CDF generalized-inverse interval and the fraction of test points on which its membership differs from raw score-set membership. Width and the interval score
\[
  ...
  \frac{2}{\alpha}(Y-U)_+
\]
are computed from the projected interval \([L,U]\) \cite{gneiting2007}. Mean and worst group gaps are descriptive absolute deviations from 0.90 across ten equal-count input bins; they are not finite-sample conditional-coverage guarantees.

Two additional diagnostics probe the proposed mechanism. At each test point,
the raw relative scale error is
\[
  \abs{\widehat s(X)/s(X)-1},
\]
which is averaged over test points and then over macroreplications. Because the
benchmark does not divide by \(\kappa_{10}\) or retune \(c\), this quantity
includes global-multiplier error and is not the \(\Delta_n\) in
Assumption~\ref{ass:scale}. For score homogeneity, each macroreplication
partitions the test inputs into ten equal-count bins, computes the 45 pairwise
empirical Kolmogorov--Smirnov distances between raw-score distributions, and
records their maximum. The reported value averages this maximum over
macroreplications. It is a relative diagnostic without a permutation or null
baseline; taking a maximum over 45 finite-sample comparisons makes a positive
value expected even under exact homogeneity.

\subsection{Marginal validity and main performance comparison}

Table~\ref{tab:final-adaptive-h} reports the main \(B=1000\) results. Raw-score marginal coverage ranges from 0.898 to 0.902 across all nine DGP--method combinations, with coverage Monte Carlo standard errors near 0.002. Thus the matched-target split-conformal score sets retain the intended marginal coverage across bandwidth rules.

\input{../generated/tables/adaptive_h/final_adaptive_h_B1000_table.tex}

Raw score-set coverage must not be read as coverage of the projected reporting
interval. For M/M/1 with fixed bandwidth, projected-interval coverage is
\(0.855\), compared with raw coverage \(0.899\), and the membership
disagreement rate is \(0.049\). For the other eight \(B=1000\) combinations,
projected-interval coverage lies between \(0.900\) and \(0.903\), and
disagreement lies between \(0.003\) and \(0.009\). Hence the finite-sample
guarantee belongs to the raw score set; the projected interval and its
width and interval score are explicitly reporting-layer summaries.

The oracle mechanism is clearest on the two raised-floor models. Relative to fixed bandwidth, the oracle reduces the maximum pairwise score-distribution KS diagnostic from \(0.531\) to \(0.296\) under Gaussian noise and from \(0.718\) to \(0.437\) under Student-\(t_3\) noise. The corresponding worst-group gaps fall from \(0.112\) to \(0.084\) and from \(0.112\) to \(0.088\). Oracle interval score also improves from \(3.334\) to \(3.198\) and from \(3.576\) to \(3.373\), respectively.

The M/M/1 case is an important boundary. Fixed and oracle score KS diagnostics are similar (\(0.224\) and \(0.231\)), while the oracle interval score is worse (\(12.463\) versus \(10.983\)) despite its narrower interval. This comparison must be interpreted together with the fixed arm's projected-interval undercoverage and score-set disagreement. It shows that population scale normalization alone does not guarantee practical improvement for every DGP, fixed multiplier, training budget, and interval-projection rule.

\subsection{What the implementable plug-in tracks}

The raw relative scale error declines as Stage-1 information grows. From \(B=100\) to \(B=1000\), it decreases from \(0.182\) to \(0.075\) for M/M/1, from \(0.920\) to \(0.353\) for the Gaussian model, and from \(0.817\) to \(0.308\) for the Student-\(t_3\) model. These values show increasing, but still partial, tracking of the local scale; they do not establish the uniform consistency assumed in the theory. Figure~\ref{fig:effective-ratio} shows the corresponding stabilization of \(h(x)/s(x)\).

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{final_effective_bandwidth_ratio.pdf}
\caption{Effective bandwidth ratio \(h(x)/s(x)\) at \(B=1000\). Curves average macroreplication-level bin means; error bars are Monte Carlo standard errors. With fixed \(r_0\), the plug-in may differ from the oracle by a global multiplier as well as by finite-sample smoothing error.}
\label{fig:effective-ratio}
\end{figure}

On the raised-floor models, the plug-in moves the raw-score diagnostic in the oracle direction. At \(B=1000\), its maximum pairwise KS is \(0.468\) for Gaussian noise and \(0.539\) for Student-\(t_3\) noise, between the corresponding fixed and oracle values. This pattern is consistent with the scale-normalization mechanism but is not a calibrated test of exact homogeneity. The improvement is not universal: on M/M/1, the plug-in KS is \(0.286\), above both fixed and oracle.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{final_raw_score_homogeneity.pdf}
\caption{Maximum pairwise empirical KS distance between raw-score distributions in ten equal-count input bins. Points are means over 50 macroreplications and error bars are Monte Carlo standard errors. This is a mechanism diagnostic, not a coverage guarantee.}
\label{fig:score-homogeneity}
\end{figure}

Practical interval performance is similarly mixed. Paired plug-in-minus-fixed mean width differences (MCSEs) are \(-0.883\) (\(0.064\)), \(-0.124\) (\(0.007\)), and \(-0.209\) (\(0.011\)) for M/M/1, Gaussian, and Student-\(t_3\), respectively. The corresponding paired interval-score differences are \(1.585\) (\(0.072\)), \(0.038\) (\(0.009\)), and \(-0.090\) (\(0.010\)); lower is better. Thus the plug-in produces narrower projected intervals in all three examples, but improves interval score only for Student-\(t_3\). Its worst raw-score group gaps, \(0.127\), \(0.117\), and \(0.068\) for Gaussian, Student-\(t_3\), and M/M/1, also do not uniformly retain the oracle improvements.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{final_binwise_coverage.pdf}
\caption{Raw-score coverage across ten equal-count input bins at \(B=1000\). Lines average macroreplication-level bin coverage; error bars are Monte Carlo standard errors. The horizontal line is the 0.90 marginal target.}
\label{fig:binwise-coverage}
\end{figure}

\subsection{Numerical audit and interpretation}

The final audit contains all 600 DGP--budget--macroreplication jobs, all 1,800 arm-level per-point files, identical paired test data, finite metrics, complete score diagnostics, and hash-bound publication assets. One warning remains in the reporting layer: two of 1,800 arm files exceed a 2\% projected-interval grid-boundary rate, both for the fixed arm at \(B=100\); the maximum is 30.1\% in one macroreplication. The mean boundary rate across arm files is \(2.14\times10^{-4}\), and no \(B=1000\) arm has a boundary hit. Raw-score coverage is unaffected.

The final evidence therefore supports three distinct conclusions. First, matched iid target-law calibration delivers the intended marginal coverage for the raw score sets, while projected-interval coverage must be audited separately. Second, oracle response-scale normalization substantially reduces the score-heterogeneity diagnostic in the two clean raised-floor location-scale examples, but not in the finite-sample M/M/1 experiment. Third, the sample-SD plus Nadaraya--Watson method increasingly tracks the response scale and can move the score diagnostic toward the oracle, yet the current untuned \(c=1\) implementation does not provide a uniform interval-score or groupwise-coverage improvement. These distinctions keep the empirical claim aligned with the theory and identify global multiplier selection, scale-smoother refinement, projection-aware interval construction, a KS null benchmark, and protocol-matched external baselines as the main next questions.
...
\[
  ...
  &\text{scale-adaptive CKME targets the DCP score used for efficiency.}
  \end{aligned}
\]
...
\ifincludeexperiments
The present experiments focus on calibrated score-set and projected-interval performance rather than raw CDF estimation alone. Raw CDF and score diagnostics are useful for mechanism checks, but raw score-set coverage, projected-interval coverage, width, interval score, and groupwise stability must remain distinct. The numerical evidence supports oracle scale normalization as a mechanism on the two raised-floor models and shows that the implementable estimator increasingly tracks their scale patterns. Its downstream interval and groupwise effects remain DGP-dependent, especially for M/M/1. Region-of-interest targets, projection-aware interval construction, fair scalar-multiplier selection, a null-calibrated score diagnostic, and refreshed external conformal or simulation-UQ baselines are necessary next steps before a journal-level performance claim.
\fi
...
Let \(\Pi(u)=\operatorname{clip}_{[0,1]}(u)\) and
\(A(u)=|u-1/2|\). The map \(A\) is one-Lipschitz, \(\Pi\) is
nonexpansive, and \(F^\circ(t\mid x)\in[0,1]\). On the event in
...
\begin{aligned}
  ...
  &=
  \abs{
  A\!\left(\Pi\{\widehat F(y\mid x)\}\right)
  -
  A\!\left(F^\circ(y\mid x)\right)
  }  \\
  &\le
  \abs{\Pi\{\widehat F(y\mid x)\}-F^\circ(y\mid x)}\\
  &\le
...
For the threshold statement, let \(\widehat G_n\) and \(G_n^\circ\) be the
empirical CDFs of the fitted and oracle calibration scores, respectively,
and let
\[
  \widehat\delta_{\Tcal}
  =
  ...
  \ind\{Y_i^{\mathrm{cal}}\notin\Tcal\}.
\]
The in-range score bound implies, for every \(u\),
\[
  G_n^\circ(u-\epsilon_n)-\widehat\delta_{\Tcal}
  \le
  \widehat G_n(u)
  \le
  G_n^\circ(u+\epsilon_n)+\widehat\delta_{\Tcal}.
...
Moreover,
\(\widehat\delta_{\Tcal}
=\delta_{\Tcal}+\Op(n_{\mathrm{cal}}^{-1/2})\), and the empirical oracle
score CDF differs uniformly from its population CDF by
\(\Op(n_{\mathrm{cal}}^{-1/2})\). Since the oracle score density is bounded
away from zero near \(q^\circ_{1-\alpha}\), inversion of the preceding
bracketing inequalities, including the \(O(n_{\mathrm{cal}}^{-1})\)
finite-sample quantile correction, gives
\[
  \abs{\widehat q_{1-\alpha}-q^\circ_{1-\alpha}}
  =
  \Op\!\left(
...
\[
  ...
  =
  \frac{z_x(t)-\varepsilon}{c\kappa_{r_0}}
  \frac{\kappa_{r_0}s(x)}{\widehat s(x)},
  \qquad
  \frac{t-Y}{c\kappa_{r_0}s(x)}
  =
  \frac{z_x(t)-\varepsilon}{c\kappa_{r_0}}.
\]
...
\[
  \sup_{x\in\Xcal}
  \abs{\frac{\kappa_{r_0}s(x)}{\widehat s(x)}-1}
  =
  \sup_{x\in\Xcal}
  \abs{\frac{1}{\widehat s(x)/\{\kappa_{r_0}s(x)\}}-1}
  \le
  2\Delta_n .
\]
...
\begin{aligned}
  ...
  \E\left[
  L_\Psi
  \abs{\frac{z_x(t)-\varepsilon}{c\kappa_{r_0}}}
  \abs{\frac{\kappa_{r_0}s(x)}{\widehat s(x)}-1}
  \middle|X=x
  \right]  \\
  &\quad\le
  \frac{2L_\Psi}{c\kappa_{r_0}}\Delta_n
  \E\left[
...
\[
  ...
  \abs{F_{\widehat h}^{\circ}(t\mid x)-F_{\bar c}^\circ(t\mid x)}
  \le
  \frac{2L_\Psi M_{\Tcal}}{c\kappa_{r_0}}\Delta_n
\]
with probability tending to one. Assumption~\ref{ass:scale} gives the result.

\subsection{Proof of Proposition~\ref{prop:ckme-exact-cdf}}

For every \(x\in\Xcal\) and \(t\in\Tcal\), the triangle inequality gives
\[
\begin{aligned}
  \abs{\widehat F_{h(\cdot)}(t\mid x)-F(t\mid x)}
  &\le
  \abs{\widehat F_{h(\cdot)}(t\mid x)
  -F_{h(\cdot)}^\circ(t\mid x)}\\
  &\quad+
  \abs{F_{h(\cdot)}^\circ(t\mid x)-F(t\mid x)}.
\end{aligned}
\]
The reproducing property and Cauchy--Schwarz inequality bound the first term by
\[
  \norm{g_{t,h(x)}}_{\Hcal_Y}
  \norm{\widehat\mu_{Y\mid x}-\mu_{Y\mid x}}_{\Hcal_Y}.
\]
...
\[
\begin{aligned}
  &F_{h(\cdot)}^\circ(t\mid x)-F(t\mid x)\\
  &\quad=
  \int_{\R}
  \left[
  \Psi\!\left(\frac{t-y}{h(x)}\right)-\ind\{y\le t\}
  \right]
  f(y\mid x)\,dy.
\end{aligned}
\]
The substitution \(u=(t-y)/h(x)\), followed by
Assumption~\ref{ass:smoothing-bias}, gives
\[
  \abs{F_{h(\cdot)}^\circ(t\mid x)-F(t\mid x)}
  \le
  B h(x)
  \int_{\R}\abs{\Psi(u)-\ind\{u\ge0\}}\,du
  =
  B C_\Psi h(x).
\]
Taking suprema over \(x\) and \(t\) yields the stated bound.
...
\]
Write \(\Tcal_n=\Tcal\) in the compact-support or global-uniform cases.
By Proposition~\ref{prop:ckme-cdf} and Proposition~\ref{prop:error-transfer}, there exists a sequence \(r_n=\op(1)\), including the marginal tail term when expanding response ranges are used, such that, with probability tending to one,
\[
  \sup_{x\in\Xcal,\ y\in\Tcal}
  \abs{\widehat S(x,y)-S_c^\circ(x,y)}\le r_n,
  \qquad
  \abs{\widehat q_{1-\alpha}-q^\circ_{1-\alpha}}\le r_n.
\]
On this event and on \(\{Y\in\Tcal_n\}\), for any group \(\Gcal_k\),
\[
\begin{aligned}
...
\begin{aligned}
  ...
  +
  \Pp(Y\notin\Tcal_n\mid X\in\Gcal_k),
\end{aligned}
\]
where \(G_k\) is the groupwise oracle score CDF in Assumption~\ref{ass:group}. For compact support or global uniform control, the tail term is zero. Otherwise it vanishes uniformly over the fixed partition by the range condition in the theorem. The common Lipschitz condition then makes the right-hand side \(\op(1)\), uniformly over the fixed finite set of groups.
...
Let \(F_{\widehat h}^{\circ}\) denote the plug-in smoothed oracle CDF and let \(F_{\bar c}^\circ\) denote the oracle adaptive smoothed CDF with \(\bar c=c\kappa_{r_0}\). By the assumed uniform CKME consistency for \(F_{\widehat h}^{\circ}\),
\[
  \sup_{x,t}
...
\[
  \sup_{x,t}
  \abs{\widehat F_{h(\cdot)}(t\mid x)-F_{\bar c}^\circ(t\mid x)}
  =
  \op(1).
\]
Thus Assumption~\ref{ass:uniform} holds with \(F^\circ=F_{\bar c}^\circ\), up to replacing \(\epsilon_n\) by the sum of the CKME and plug-in scale errors. The proof of Theorem~\ref{thm:group}, with \(c\) replaced by \(\bar c\), then applies verbatim.
...
\bibitem{hardle1990}
W.~H\"ardle.
\newblock \emph{Applied Nonparametric Regression}.
\newblock Cambridge University Press, 1990.

\bibitem{hore2025}
R.~Hore and R.~F. Barber.
\newblock Conformal prediction with local weights: Randomization enables robust guarantees.
\newblock \emph{Journal of the Royal Statistical Society Series B: Statistical Methodology}, 87(2):549--578, 2025.
...
\bibitem{papadopoulos2011}
H.~Papadopoulos, V.~Vovk, and A.~Gammerman.
\newblock Regression conformal prediction with nearest neighbours.
\newblock \emph{Journal of Artificial Intelligence Research}, 40:815--840, 2011.
