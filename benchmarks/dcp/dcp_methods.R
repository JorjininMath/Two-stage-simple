...
dcp.qr <- function(Y0,X0,Y1,X1,Y.test,X.test,taus,alpha.sig){
  ...
  return(list(cov.qr=cov.qr,leng.qr=leng.qr,lb.qr=lb,ub.qr=ub))
...
dcp.opt <- function(Y0,X0,Y1,X1,Y.test,X.test,taus,alpha.sig){
  ...
  return(list(cov.opt=cov.opt,leng.opt=leng.opt,lb.opt=lb.opt,ub.opt=ub.opt))  
...
dcp.dr <- function(Y0,X0,Y1,X1,Y.test,X.test,ys,taus,alpha.sig){
  ...
  return(list(cov.dr=cov.dr,leng.dr=leng.dr,lb.dr=lb,ub.dr=ub))
