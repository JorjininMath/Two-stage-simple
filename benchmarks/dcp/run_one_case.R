...
#
# Usage: Rscript benchmarks/dcp/run_one_case.R <data_dir> <output_csv> <alpha> <n_grid>
#############################################################################

library(hetGP)
library(quantreg)

# The benchmark implementation lives beside this runner, so it is independent
# of the shell working directory and of the Python package layout.
cmd_args <- commandArgs(trailingOnly = FALSE)
script_path <- sub("^--file=", "", cmd_args[grep("^--file=", cmd_args)])
if (length(script_path) > 0) {
  benchmark_dir <- dirname(normalizePath(script_path[1]))
} else {
  benchmark_dir <- getwd()
}
source(file.path(benchmark_dir, "dcp_methods.R"))
...
if (length(args) < 4) {
  cat("Usage: Rscript benchmarks/dcp/run_one_case.R <data_dir> <output_csv> <alpha> <n_grid>\n")
  quit(status = 1)
}
