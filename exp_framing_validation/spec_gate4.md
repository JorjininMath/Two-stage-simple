...
`Two_stage/site_selection.py`). The arms differ only in `w`:
...
(reuse `CKME/tuning.py` with `ParamGrid(ell_x_list=[0.02, 0.05, 0.1],
...
```text
exp_framing_validation/output_gate4a/
...
exp_framing_validation/output_gate4b/
...
```text
exp_framing_validation/gate4_fix_epistemic.py
...
`manuscript/journal_scale_adaptive/paper_storyline.md` Section 6.
