"""
experiments/nongauss: Compare CKME, DCP-DR, hetGP on six non-Gaussian noise DGPs.
...
Usage (from project root):
  python experiments/nongauss/run_nongauss_compare.py
  python experiments/nongauss/run_nongauss_compare.py --n_macro 10 --method mixed
  python experiments/nongauss/run_nongauss_compare.py --h_mode adaptive --c_scale 2.0
"""
...
import numpy as np
import pandas as pd
from experiments.nongauss.config_utils import load_config_from_file, get_config, get_x_cand
...
R_SCRIPT = _root / "benchmarks" / "dcp" / "run_one_case.R"
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=str(_EXPERIMENT_DIR / "config.txt"),
    )
    ...
    if args.output_dir:
        ...
        out_dir = _EXPERIMENT_DIR / f"output_adaptive_c{args.c_scale:.2f}"
    else:
        out_dir = _EXPERIMENT_DIR / "output"
    ...
    pretrained_path = _EXPERIMENT_DIR / "pretrained_params.json"
    pretrained: dict = {}
    if pretrained_path.exists():
        ...
        print(
            ...
            "Run 'python experiments/nongauss/pretrain_params.py' first to improve accuracy.",
            file=sys.stderr,
        )
