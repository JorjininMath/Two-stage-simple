...
#   Rscript experiments/gibbs_compare/run_rlcp_gibbs.R [--nrep 20] [--h 0.05]
...
# ---- paths ----------------------------------------------------------------
# Resolve from this script so direct execution does not depend on the shell's
# current directory.  RLCP is preserved as an external reproduction archive.
script_arg <- grep("^--file=", commandArgs(trailingOnly = FALSE), value = TRUE)
if (length(script_arg) != 1) stop("Could not resolve run_rlcp_gibbs.R path")
script_path <- normalizePath(sub("^--file=", "", script_arg), mustWork = TRUE)
experiment_dir <- dirname(script_path)
proj_root <- normalizePath(file.path(experiment_dir, "..", ".."), mustWork = TRUE)
utils_dir <- file.path(proj_root, "_archive", "04_external_reproductions",
                       "rlcp", "RLCP", "utils")
out_dir   <- file.path(experiment_dir, "output")
