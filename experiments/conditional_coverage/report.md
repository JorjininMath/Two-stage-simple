...
separate experiments (`experiments/nongauss`, `exp_branin`). Adding them here would force
...
This is consistent with the broader observation in `experiments/onesided` that the CKME
...
- **Simulators are 1-dimensional and Gaussian.** Both exp1 and exp2 use heteroscedastic
  Gaussian noise. Non-Gaussian consistency is studied separately in `experiments/nongauss`;
...
- **Two-sided abs-median score only.** The CP score `|F̂ − 0.5|` is symmetric.
  One-sided consistency is evaluated in `experiments/onesided`.
...
- **L1 errors only at τ ∈ {α/2, 1−α/2} = {0.05, 0.95}.** Tail behaviour at other τ
  may differ; see `experiments/onesided` for the full τ-scan.
