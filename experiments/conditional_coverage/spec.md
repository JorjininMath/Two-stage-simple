...
(against DCP-DR, hetGP) is done in separate experiments (experiments/nongauss, exp_branin).
...
```
experiments/conditional_coverage/output_{h_mode}/
...
- The CP interval uses abs-median score, which is two-sided. One-sided consistency is not
  evaluated here; see experiments/onesided.
- L1 quantile errors are computed at τ = α/2 and 1−α/2 only. Performance at other τ
  values (especially tail quantiles) may differ; see experiments/onesided for the full τ-scan.
