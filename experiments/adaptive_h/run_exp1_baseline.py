"""
experiments/adaptive_h / Exp1: fixed-h baseline on four DGPs.
...
Outputs:
    experiments/adaptive_h/output_exp1/
        macrorep_{k}/case_{sim}/per_point.csv
        exp1_summary.csv        (mean coverage / width / interval-score per DGP)

Usage (from project root):
    python experiments/adaptive_h/run_exp1_baseline.py
    python experiments/adaptive_h/run_exp1_baseline.py --n_macro 50 --n_workers 8
"""
...
# to avoid right-tail truncation in CDF inversion. See
# notes/ideas/archived/sample-scale-plugin-alternative-estimators.md.
T_GRID_MARGIN = {
    "wsc_gauss":    0.30,
    "gibbs_s1":     0.30,
...
def run_one_macrorep(
    ...
    # t_grid truncation diagnostics (Method 1 in the archived alternatives note)
...
def main():
    ...
    out_dir = Path(args.output_dir) if args.output_dir else _root / "experiments" / "adaptive_h" / "output_exp1"
    out_dir.mkdir(parents=True, exist_ok=True)

    pretrained_path = _root / "experiments" / "adaptive_h" / "pretrained_params.json"
