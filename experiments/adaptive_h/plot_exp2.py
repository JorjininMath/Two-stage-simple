...
Reads:
    experiments/adaptive_h/output_exp2/macrorep_{k}/case_{sim}_{fixed|oracle}/per_point.csv

Writes:
    experiments/adaptive_h/output_exp2/exp2_coverage_curves.png

Usage (from project root):
    python experiments/adaptive_h/plot_exp2.py
    python experiments/adaptive_h/plot_exp2.py --output_dir experiments/adaptive_h/output_exp2 --n_bins 25
"""
...
def _draw_panel(ax, out_dir: Path, sim: str, n_bins: int, target: float, show_title: bool = True):
    ...
    for arm in ["fixed", "oracle"]:
        ...
        cov_per = np.empty((len(case_paths), n_bins))
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="experiments/adaptive_h/output_exp2")
    parser.add_argument("--n_bins",     type=int, default=20)
    parser.add_argument("--alpha",      type=float, default=0.1)
    parser.add_argument("--out_png",    type=str, default=None)
    parser.add_argument(
        "--simulator", type=str, default=None,
        choices=SIMULATORS,
        help="If set, render only this single DGP as a standalone figure (no title).",
    )
    ...
    if args.simulator is not None:
        ...
        _draw_panel(ax, out_dir, sim, args.n_bins, target, show_title=False)
        fig.tight_layout()
        out_png = Path(args.out_png) if args.out_png else out_dir / f"exp2_coverage_curves_{sim}.png"
    ...
    for ax, sim in zip(axes, SIMULATORS):
        _draw_panel(ax, out_dir, sim, args.n_bins, target, show_title=True)
