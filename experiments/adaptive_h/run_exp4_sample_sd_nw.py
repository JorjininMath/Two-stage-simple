"""Exp4: sample-SD/NW adaptive h(x) vs fixed h and oracle adaptive h(x).
...
once, then evaluate THREE arms on the same X_test / Y_test:
  ...
  - "plugin"  : adaptive h(x) = c * sigma_hat(x), where sigma_hat(x) is built
                from per-site sample SD plus Nadaraya-Watson smoothing.  The
                legacy arm label is retained so existing result tables remain
                readable; it never denotes the retired IQR response-scale
                estimator.
...
Outputs:
    experiments/adaptive_h/output_exp4/
...
Usage (from project root):
    python experiments/adaptive_h/run_exp4_sample_sd_nw.py --n_macro 50 --n_workers 6
    python experiments/adaptive_h/run_exp4_sample_sd_nw.py --budgets 50,100,250,500 --r0_fixed 10
"""
...
from experiments.adaptive_h.config_utils import load_config_from_file, get_config, get_x_cand
...
def run_one_macrorep(
    ...
    # The on-disk arm label remains "plugin" for compatibility with existing
    # paired summaries.  The estimator itself is explicitly sample-SD + NW.
    scale_estimator = SampleSdNwScaleEstimator.fit(
        stage1.X_all,
        stage1.Y_all,
        n_0,
        r_0,
        bw_factor=scale_bw_factor,
    )
    h_cal_pl = scale_estimator.get_h(stage2.X_stage2, c_scale)
    h_test_pl = scale_estimator.get_h(X_test, c_scale)
    ...
    arm_rows.append({
        ...
        "scale_estimator": "sample_sd_nw",
        "scale_bw_factor": float(scale_bw_factor),
    })
...
def main():
    parser = argparse.ArgumentParser(
        description="Exp4: sample-SD/NW vs fixed vs oracle adaptive h"
    )
    parser.add_argument("--config",     type=str, default="experiments/adaptive_h/config.txt")
    ...
    parser.add_argument(
        "--scale_bw_factor",
        "--plugin_bw_factor",
        dest="scale_bw_factor",
        type=float,
        default=1.0,
        help=(
            "Multiplier on the X-space NW smoothing bandwidth. "
            "The old option name is accepted for command compatibility."
        ),
    )
    ...
    parser.add_argument("--pretrained_path", type=str, default=None,
                        help="Path to pretrained params JSON. Defaults to experiments/adaptive_h/pretrained_params.json.")
    ...
    # User-provided relative paths are project-root relative, independent of
    # the shell's current directory.
    out_dir = (
        (_root / args.output_dir).resolve()
        if args.output_dir
        else Path(__file__).resolve().parent / "output_exp4"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    pretrained_path = (
        (_root / args.pretrained_path).resolve()
        if args.pretrained_path
        else _root / "experiments" / "adaptive_h" / "pretrained_params.json"
    )
    raw = {}
    if not pretrained_path.exists() and not args.allow_default_params:
        print(
            f"ERROR: {pretrained_path} not found.\n"
            "Run 'python experiments/adaptive_h/pretrain_params.py' first.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        ...
        f"scale_bw_factor={args.scale_bw_factor}, simulators={simulators}, "
        ...
        for budget_idx, B in enumerate(budgets):
            ...
            jobs = [
                (k, args.base_seed, config, sim, B, args.r0_fixed,
                 ...
                 args.scale_bw_factor, args.stage1_design, sim_idx, budget_idx)
