"""
plot_exp4_score_homogeneity.py
...
Reads:
    experiments/adaptive_h/output_exp4/exp4_paired_deltas.csv
    experiments/adaptive_h/output_exp4/macrorep_*/budget_*/case_nongauss_A1L_*/per_point.csv

Writes:
    experiments/adaptive_h/output_exp4/exp4b_coverage_curves.png
    experiments/adaptive_h/output_exp4/exp4b_qhat_ratio_vs_budget.png

Usage (from project root):
    python experiments/adaptive_h/plot_exp4_score_homogeneity.py
    python experiments/adaptive_h/plot_exp4_score_homogeneity.py --output_dir experiments/adaptive_h/output_exp4 --c_scale 1.0
"""
...
def _figure2_qhat_ratio(out_dir: Path, simulator: str, paired: pd.DataFrame, c_scale: float):
    ...
    # Population limit of conditional SD / scale: 1 for Gaussian DGPs and
    # sqrt(nu / (nu - 2)) for Student-t_nu with nu > 2.
    if simulator == "nongauss_A1L":
        nu = 3.0
        h_target = float(np.sqrt(nu / (nu - 2.0)))
        h_target_label = rf"$\sqrt{{\nu/(\nu-2)}}\approx{h_target:.3f}$ (asymp.)"
    else:
        h_target = 1.0
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="experiments/adaptive_h/output_exp4")
    ...
    parser.add_argument("--c_scale",    type=float, default=1.0,
                        help="c used in run_exp4_sample_sd_nw (for plateau ref line).")
    ...
    if not paired_path.exists():
        print(f"ERROR: {paired_path} not found. Run summarize_exp4_sample_sd_nw.py first.",
              file=sys.stderr)
        sys.exit(1)
