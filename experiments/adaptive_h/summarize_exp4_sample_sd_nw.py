"""
summarize_exp4_sample_sd_nw.py
...
Reads:
    experiments/adaptive_h/output_exp4/exp4_per_arm.csv          (one row per macrorep,sim,budget,arm)
    experiments/adaptive_h/output_exp4/macrorep_*/budget_*/case_*/per_point.csv
                                                          (per-test-point detail)

Writes:
    experiments/adaptive_h/output_exp4/exp4_summary_arm.csv      (mean/sd of metrics per cell)
    experiments/adaptive_h/output_exp4/exp4_paired_deltas.csv    (per macrorep paired comparisons)
    experiments/adaptive_h/output_exp4/exp4_paired_summary.csv   (median/IQR of paired deltas)
    experiments/adaptive_h/output_exp4/exp4_table.tex            (LaTeX summary)

Usage (from project root):
    python experiments/adaptive_h/summarize_exp4_sample_sd_nw.py
    python experiments/adaptive_h/summarize_exp4_sample_sd_nw.py --output_dir experiments/adaptive_h/output_exp4 --alpha 0.1
"""
...
def main():
    ...
    if not per_arm_path.exists():
        print(f"ERROR: {per_arm_path} not found. Run run_exp4_sample_sd_nw.py first.",
              file=sys.stderr)
        sys.exit(1)
