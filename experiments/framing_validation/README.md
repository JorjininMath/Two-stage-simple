# experiments/framing_validation

Mechanism-validation gates for the aleatoric/epistemic framing of the
journal paper (see
`notes/planning/current/paper/paper-storyline-and-merge-decision.md`,
...
|------|--------|-------|-------------|
| G1 | `gate1_coverage_vs_scale.py` | Fixed-h bin coverage varies systematically with noise scale s(x); oracle h(x) flattens it | post-hoc on `experiments/adaptive_h/output_exp2` (50 macroreps, no new simulation) |
...
python experiments/framing_validation/gate2_score_homogeneity.py --n_macro 10 --n_workers 4
python experiments/framing_validation/gate2_score_homogeneity.py --analyze_only   # re-plot only
python experiments/framing_validation/gate3_epistemic_diagnostics.py
python experiments/framing_validation/gate4_fix_epistemic.py --part all --n_macro 20
python experiments/framing_validation/gate4_fix_epistemic.py --analyze_only   # re-plot + summary only
```
